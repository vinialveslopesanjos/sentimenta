...
function RunCard({ run }: { run: PipelineRun }) {
  ...
  const ptColor =
    platformColors[run.platform?.toLowerCase() ?? ""] ??
    "from-violet-50 to-violet-100 text-violet-500";

  return (
    <div
      className={`dream-card border ${cfg.border} ${cfg.bg} p-6 transition-all duration-300 hover:shadow-float`}
    >
      {/* Top row */}
      <div className="flex items-start justify-between gap-4 mb-4">
        <div className="flex items-center gap-3 min-w-0">
          ...
          {run.platform && (
            <div
              className={`w-10 h-10 rounded-2xl bg-gradient-to-tr ${ptColor} flex items-center justify-center shrink-0`}
            >
          ...
          <div className="min-w-0">
            <p className="text-sm font-medium text-slate-700 truncate">
              {run.connection_username ? `@${run.connection_username}` : "Pipeline"}
            </p>
            <p className="text-xs text-slate-400 font-light capitalize">
              {run.platform ?? "Sistema"} · {run.run_type}
            </p>
          </div>
        </div>
        ...
        {/* Status badge */}
        <span
          className={`inline-flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-wide px-2.5 py-1 rounded-full border shrink-0 ${cfg.badgeBg} ${cfg.badgeText} ${cfg.badgeBorder}`}
        >
          <span
            className={`w-1.5 h-1.5 rounded-full ${cfg.dot} ${cfg.pulse ? "animate-pulse" : ""} shrink-0`}
          />
          {cfg.label}
        </span>
      </div>
        ...
        {run.status === "running" ? (
          ...
          <>
            <StatPill
              icon="timer"
              label="Duração"
              value={duration ?? (run.status === "running" ? "…" : "—")}
            />
            <StatPill icon="attach_money" label="Custo" value={fmtCost(run.total_cost_usd)} />
          </>
        )}
      </div>
      ...
      {/* Notes */}
      {run.notes && (
        <p className="text-xs text-slate-400 font-light italic bg-slate-50 rounded-xl px-3 py-2 mb-4">
          {run.notes}
        </p>
      )}
      ...
      <div className="flex flex-wrap items-center gap-4 text-[10px] text-slate-300 font-light border-t border-slate-50 pt-3">
        ...
        <span className="ml-auto font-mono text-[9px] text-slate-200 truncate max-w-[120px]">
          #{run.id.slice(0, 8)}
        </span>
      </div>
    </div>
...
export default function LogsPage() {
  ...
  // Auto-refresh every 5s while there are running pipelines
  useEffect(() => {
        ...
        {!loading && runs.length > 0 && (
          <div className="flex flex-wrap gap-3">
            {[
              ...
              { icon: "attach_money", label: "Custo total", value: fmtCost(totalCost), color: "bg-amber-50 text-amber-700 border-amber-100" },
            ].map((s) => (
              <div
                key={s.label}
                className={`flex items-center gap-2 px-4 py-2 rounded-2xl border text-sm font-medium ${s.color}`}
              >
        ...
        {isEmpty && (
          <div className="flex flex-col items-center justify-center py-32 text-center">
            ...
            <p className="text-sm text-slate-400 font-light max-w-sm leading-relaxed">
              Conecte um perfil e clique em{" "}
              <span className="font-medium text-brand-lilacDark">Analisar</span>{" "}
              para iniciar o pipeline de análise de sentimentos.
            </p>
          </div>
        )}
